...
<style>
	...
	<div class="col-md-8 col-md-offset-2">
		...
		<h3 class="text-center">Redaguoti audinius</h3>
            @include('errors.list')
        <form enctype="multipart/form-data" id="fabrics-form" method="POST" action="/admin/fabrics/{!! $fabric->id !!}">
        {{ method_field('PUT') }}
			{{ csrf_field() }}
            ...
            <div class="form-group">
                ...
                <select name="category" id="" class="form-control">
                    ...
                    @if($fabric->category_id == $category->id)
                    <option selected value="{!! $category->id !!}">{!! $category->title_lt !!}</option>
                    @else
                ...
                <div class="form-group">
                    ...
                    <input type="text" name="title_lt" id="title_lt" class="form-control" value="{!! $fabric->title_lt !!}" 
                           required>
                </div>

                    <div class="form-group">
                    <label for="username">Pavadinimas EN: </label>
                    <input type="text" name="title_en" id="title_en" class="form-control" value="{!! $fabric->title_en !!}" 
                           required>
                    </div>

                    <div class="form-group">
                           <label for="username">Pavadinimas RU: </label>
                    <input type="text" name="title_ru" id="title_ru" class="form-control" value="{!! $fabric->title_ru !!}" 
                           required>
                </div>

                <div class="form-group">
                    <label for="body">Tekstas LT: </label>
                    <textarea class="form-control" rows="8" name="text_lt" id="text_lt" >{!! $fabric->text_lt !!}</textarea>
                    </div>

                    <div class="form-group">
                    <label for="text">Tekstas EN: </label>
                    <textarea class="form-control" rows="8" name="text_en" id="text_en" >{!! $fabric->text_en !!}</textarea>
                    </div>

                    <div class="form-group">
                    <label for="text">Tekstas RU: </label>
                    <textarea class="form-control" rows="8" name="text_ru" id="text_ru" >{!! $fabric->text_ru !!}</textarea>
                </div>

                   
            <ul id="fabrics_list" class="list-group">
                @php $i = 0; @endphp
                @foreach($fabric->images as $image)
                    <li class="list-group-item">
                        <img src="/{!! $image->image_path.$image->image_name !!}" width="70">
                        <br>
                        <label for="fabric_images_{!! $fabric->id !!}">
                        <input id="fabric_image" type="checkbox" name="photo_data[{!! $image->id !!}][delete]" value="1">Ištrinti
                        </label>
