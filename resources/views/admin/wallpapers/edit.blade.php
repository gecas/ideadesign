...
<style>
	...
	<div class="col-md-8 col-md-offset-2">
		...
		<h3 class="text-center">Redaguoti tapetus</h3>
            @include('errors.list')
        <form enctype="multipart/form-data" id="wallpapers-form" method="POST" action="/admin/wallpapers/{!! $wallpaper->id !!}">
        {{ method_field('PUT') }}
			{{ csrf_field() }}
            ...
            <div class="form-group">
                ...
                <select name="manufacturer" id="" class="form-control">
                    ...
                    @if($wallpaper->category_id == $category->id)
                ...
                <div class="form-group">
                    ...
                    <input type="text" name="title_lt" id="title_lt" class="form-control" value="{!! $wallpaper->title_lt !!}" 
                           required>
                </div>

                    <div class="form-group">
                    <label for="username">Pavadinimas EN: </label>
                    <input type="text" name="title_en" id="title_en" class="form-control" value="{!! $wallpaper->title_en !!}" 
                           required>
                    </div>

                    <div class="form-group">
                           <label for="username">Pavadinimas RU: </label>
                    <input type="text" name="title_ru" id="title_ru" class="form-control" value="{!! $wallpaper->title_ru !!}" 
                           required>
                </div>

                <div class="form-group">
                    <label for="body">Tekstas LT: </label>
                    <textarea class="form-control" rows="8" name="text_lt" id="text_lt" >{!! $wallpaper->text_lt !!}</textarea>
                    </div>

                    <div class="form-group">
                    <label for="text">Tekstas EN: </label>
                    <textarea class="form-control" rows="8" name="text_en" id="text_en" >{!! $wallpaper->text_en !!}</textarea>
                    </div>

                    <div class="form-group">
                    <label for="text">Tekstas RU: </label>
                    <textarea class="form-control" rows="8" name="text_ru" id="text_ru" >{!! $wallpaper->text_ru !!}</textarea>
                </div>

                <div class="form-group">
                    <label for="username">Gamintojo nuoroda: </label>
                    <input type="text" name="manufacturer_url" id="" class="form-control" value="{!! $wallpaper->manufacturer_url !!}" 
                           required>
                </div>

                   
            <ul id="wallpapers_list" class="list-group">
                @php $i = 0; @endphp
                @foreach($wallpaper->images as $image)
                    <li class="list-group-item">
                        <img src="/{!! $image->image_path.$image->image_name !!}" width="70">
                        <br>
                        <label for="wallpaper_images_{!! $wallpaper->id !!}">
                        <input id="wallpaper_image" type="checkbox" name="photo_data[{!! $image->id !!}][delete]" value="1">Ištrinti
                        </label>
                ...
                <br>
                <label for="wallpaper_images_{!! $wallpaper->id !!}">
                <input id="wallpaper_image" type="checkbox" name="photo" value="{!! $wallpaper->id !!}"> Ištrinti
                </label>
                <br>
